...
def run_autogen_pipeline():
    ...
    for pdf_file in pdf_files:
        ...
        # Extract company name from the entire PDF (assumed to be on the first page)
        company_name = company_name_agent.run(pdf_text)

        # Get candidate sections (financial data)
        ...
        # Combine the first page (for company name) with the selected section text.
        first_page = pdf_text.split("\f")[0]
        combined_text = first_page + "\n---\n" + selected_section

        # Send the combined text to the DataExtractionAgent.
