...
def main():
    ...
    for col in df.columns:
        ...
        # Other columns (e.g., CompanyName, PeriodStartEnd, FileName) remain unchanged
    
    # Fill NaNs in numeric columns with 0, especially if any "OtherOperatingExpense" are missing
    ...
    # operating_income = GrossProfit + OtherOperatingIncome - operating_expenses
    ...
    # ------------------------------------------------------------------------
    # Split "PeriodStartEnd" into "period_start" and "period_end"
    if "PeriodStartEnd" in df.columns:
        df[["period_start", "period_end"]] = df["PeriodStartEnd"].str.split(" - ", expand=True)
        df["period_start"] = pd.to_datetime(df["period_start"])
        df["period_end"] = pd.to_datetime(df["period_end"])
    else:
        # If "PeriodStartEnd" does not exist, ensure you have "period_end" in the dataset
    ...
    rename_map = {
        ...
        "FileName": "filename",
        # period_start and period_end remain the same key names
    }
    ...
    # ------------------------------------------------------------------------
    # Create a new column 'quarter' using the fiscal quarter derived from period_end
    df_final['quarter'] = df_final['period_end'].apply(get_fiscal_quarter)
    
    # ------------------------------------------------------------------------
    # 9. Sort by period_end to maintain chronological order
    # ------------------------------------------------------------------------
    df_final.sort_values("period_end", inplace=True)
    
    # Optionally, set period_end as the index if you prefer a time series index
    # df_final.set_index("period_end", inplace=True)
